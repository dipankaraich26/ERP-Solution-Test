...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add' && !empty($_POST['employee_id'])) {
            try {
                $stmt = $pdo->prepare("
                    INSERT INTO po_inspection_approvers (employee_id, is_active)
                    VALUES (?, 1)
        ...
        if ($action === 'remove' && !empty($_POST['approver_id'])) {
            try {
                $stmt = $pdo->prepare("UPDATE po_inspection_approvers SET is_active = 0 WHERE id = ?");
        ...
        if ($action === 'activate' && !empty($_POST['approver_id'])) {
            try {
                $stmt = $pdo->prepare("UPDATE po_inspection_approvers SET is_active = 1 WHERE id = ?");
                $stmt->execute([$_POST['approver_id']]);
...
$nonApprovers = $pdo->query("
    ...
    AND e.id NOT IN (SELECT employee_id FROM po_inspection_approvers WHERE is_active = 1)
...
<div class="content" style="overflow-y: auto; height: 100vh;">
    ...
    <h1>Manage PO Incoming Inspection Approvers</h1>

    <p style="margin-bottom: 20px;">
        <a href="setup_po_inspection.php" class="btn btn-secondary">Back to Setup</a>
        <a href="../stock_entry/index.php" class="btn btn-secondary">Stock Entries</a>
    </p>
    ...
    <div style="background: #f0f9ff; border: 1px solid #0ea5e9; border-radius: 8px; padding: 20px; margin-bottom: 25px;">
        <h3 style="margin: 0 0 15px 0; color: #0369a1;">Add New Inspection Approver</h3>
    ...
    <!-- Current Approvers -->
    <h2>Current Inspection Approvers</h2>
    ...
    <?php if (empty($approvers)): ?>
        <div style="background: #fef3c7; border: 1px solid #f59e0b; padding: 20px; border-radius: 8px; text-align: center; color: #92400e;">
            <p style="margin: 0;">No approvers configured yet. Add employees who can approve incoming inspection checklists.</p>
        </div>
    <?php else: ?>
    ...
    <div style="background: #e0e7ff; border: 1px solid #6366f1; border-radius: 8px; padding: 15px; margin-top: 25px;">
        ...
        <span style="color: #3730a3;">
            Only active approvers will appear in the approval request dropdown when receiving stock.
            Approvers can view and approve/reject incoming inspection checklists from the Stock Entry section.
        </span>
    </div>
</div>
