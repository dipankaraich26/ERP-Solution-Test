...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add' && !empty($_POST['employee_id'])) {
            try {
                $stmt = $pdo->prepare("
                    INSERT INTO so_approvers (employee_id, is_active)
                    VALUES (?, 1)
        ...
        if ($action === 'remove' && !empty($_POST['approver_id'])) {
            try {
                $stmt = $pdo->prepare("UPDATE so_approvers SET is_active = 0 WHERE id = ?");
        ...
        if ($action === 'activate' && !empty($_POST['approver_id'])) {
            try {
                $stmt = $pdo->prepare("UPDATE so_approvers SET is_active = 1 WHERE id = ?");
                $stmt->execute([$_POST['approver_id']]);
...
$nonApprovers = $pdo->query("
    ...
    AND e.id NOT IN (SELECT employee_id FROM so_approvers WHERE is_active = 1)
...
<div class="content">
    ...
    <h1>Manage Sales Order Approvers</h1>

    <p style="margin-bottom: 20px;">
        <a href="/sales_orders/index.php" class="btn btn-secondary">Sales Orders</a>
        <a href="/approvals/index.php" class="btn btn-secondary">My Approvals</a>
    </p>
    ...
    <?php if (empty($approvers)): ?>
        <div style="background: #fef3c7; border: 1px solid #f59e0b; padding: 20px; border-radius: 8px; text-align: center; color: #92400e;">
            <p style="margin: 0;">No approvers configured yet. Add employees who can approve sales order releases.</p>
        </div>
    <?php else: ?>
    ...
    <div style="background: #e0e7ff; border: 1px solid #6366f1; border-radius: 8px; padding: 15px; margin-top: 25px;">
        ...
        <span style="color: #3730a3;">
            Only active approvers will appear in the approval request dropdown on the SO release checklist page.
            Approvers can view and approve/reject release requests from the <a href="/approvals/index.php">My Approvals</a> page.
        </span>
    </div>
</div>
