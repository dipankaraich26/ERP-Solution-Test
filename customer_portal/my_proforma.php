...
try {
    $piStmt = $pdo->prepare("
        SELECT q.id, q.quote_no, q.pi_no, q.quote_date, q.validity_date, q.status,
...
<head>
    ...
    <title>My Proforma Invoices - Customer Portal</title>
...
<div class="portal-content">
    ...
    <?php $total = count($proformas); $total_value = array_sum(array_column($proformas, 'total_value')); ?>
    <div class="summary-bar">
        <div class="summary-item"><div class="value"><?= $total ?></div><div class="label">Total Proforma</div></div>
        <div class="summary-item"><div class="value" style="color: #9b59b6;"><?= number_format($total_value, 2) ?></div><div class="label">Total Value (INR)</div></div>
    </div>
    <?php if (empty($proformas)): ?>
        <div class="table-container"><div class="empty-state"><div class="icon">📄</div><h3>No Proforma Invoices Found</h3></div></div>
    <?php else: ?>
        <div class="table-container"><div class="table-scroll">
            <table class="data-table">
                <thead><tr><th>#</th><th>PI No</th><th>Quote No</th><th>Date</th><th>Validity</th><th class="text-right">Amount</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                <?php foreach ($proformas as $i => $pi): ?>
                <tr>
                    <td><?= $i + 1 ?></td>
                    ...
                    <td class="text-right" style="font-weight: bold;"><?= $pi['total_value'] ? number_format($pi['total_value'], 2) : '-' ?></td>
                    <td><span class="status-badge status-<?= strtolower($pi['status'] ?: 'draft') ?>"><?= htmlspecialchars($pi['status'] ?: 'Draft') ?></span></td>
                    <td style="white-space: nowrap;">
                        <a href="/quotes/print.php?id=<?= $pi['id'] ?>" class="btn" target="_blank">View</a>
                        <a href="/quotes/print.php?id=<?= $pi['id'] ?>&download=1" class="btn btn-download" target="_blank">Download</a>
                    </td>
                </tr>
                <?php endforeach; ?>
                </tbody>
